...
document.body.style.boxShadow = 'inset 0 0 300px  rgba(0, 0, 0, 0.2)';
...
resetParticles();
window.addEventListener('click', resetParticles);
...
export default function BasicFlow() {
  ...
  return (
    ...
    >
      {/* <color attach="background" args={['#4c5d77']} ref={bkRef} /> */}
      ...
      {/* <EffectComposer>
        <Vignette eskil={true} offset={0} darkness={0.3} />
      </EffectComposer> */}
    </Canvas>
  );
}
  ...
  useFrame((state, delta) => {
    ...
    for (let i = 0; i < particles.length; i++) {
      ...
      if (p.currentLife < 0) {
        ...
        // p.z = Math.cos(t * p.zSpeed + i * 10) * p.zRange;
  ...
  return (
    <group ref={ref}>
      ...
      >
        ...
        {/* This is a cool geomoetry option too */}
...
const Line = ({ index }) => {
  ...
  useFrame((state) => {
    ...
    if (particles[index].currentLife < -30) {
      return;
    }
